...
- **양자 어닐링을 적용하려면 모델을 다시 설계해야 한다.** strong connectivity는 네트워크 전체의 순환 구조에 적용되는 전역 조건이므로 저차 다항식으로 표현할 수 없다. 또한 Pegasus $P_{16}$은 고정된 희소 그래프여서 minor embedding이 먼저 한계에 도달한다.
...
| 항목 | 기존 방식 | 본 과제 |
| --- | --- | --- |
| strong connectivity 처리 | QUBO의 penalty term으로 강제 → 전역 조건이므로 저차로 표현할 수 없고, 큰 페널티가 어닐러의 유효 분해능을 낮춘다 | 전처리 단계에서 경계 사이클로 순환 골격을 만들고, 에너지 함수에는 도달 가능성 항만 남긴다 |
| 목적 함수 차수 | 도달성을 직접 사용하면 3차 이상이므로 보조 변수가 필요하다(4-hop은 변수 약 8배) | 최고차 항이 상쇄되는 $n\in\{2,3\}$만 사용해 보조 변수 없이 2차 유지 |
| 부분 문제 분해 | 제약이 없는 QUBO가 대상이므로 부분해를 단순히 합쳐도 된다 | 합친 뒤에도 strong connectivity를 유지하도록 면 단위로 나누고 거대면마다 순환 방향을 부여한다 |
| 문제 크기 | 정점 500개인 전체 QUBO는 변수가 1,481개여서 탐색 예산 안에 embedding 실패 | 분할 후 최대 부분 그래프 평균 263–365개, 정점 500개까지 모두 embedding 성공 |
...
샌드버그 이세진 이사가 2026년 8월 7일에 서면으로 자문했다(`docs/04.자문의견서/산업체_자문의견서.pdf`).
