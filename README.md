...
| 문제 크기 | 전체 그래프를 QUBO 하나로 올린다. 논리 변수 하나가 물리 큐비트 여러 개의 chain으로 올라가므로 정점 100개짜리 그래프에 이미 평균 3,008개가 필요하고, 그보다 큰 그래프는 Pegasus $P_{16}$이 제공하는 5,640개를 넘어 한 번에 올릴 수 없다 | 그래프를 부분 그래프로 나누어 하나씩 올린다. 정점 500개 그래프도 부분 그래프당 물리 큐비트 최대 4,391개로 한계 안에 들어간다 |
